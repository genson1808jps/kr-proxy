...
// bare Claude model IDs (e.g. claude-opus-4.8) can use kr/claude-opus-4.8 instead.
const kiroModelPrefix = "kr/"

// stripKiroModelPrefix removes the optional kr/ prefix (case-insensitive).
func stripKiroModelPrefix(model string) string {
	if len(model) >= len(kiroModelPrefix) && strings.EqualFold(model[:len(kiroModelPrefix)], kiroModelPrefix) {
		return model[len(kiroModelPrefix):]
	}
	return model
}
...
func ParseModelAndThinking(model string, thinkingSuffix string) (string, bool) {
	model = stripKiroModelPrefix(model)
...
// resolveOpenAIThinkingMode reports whether extended reasoning should be enabled
// for an OpenAI Chat Completions request (model suffix, reasoning_effort, or
// model-name hints).
func resolveOpenAIThinkingMode(req *OpenAIRequest, thinkingSuffix string) (string, bool) {
	actualModel, thinking := ParseModelAndThinking(req.Model, thinkingSuffix)
	if thinking {
...
func OpenAIToKiro(req *OpenAIRequest, thinking bool) *KiroPayload {
	...
	for i, msg := range nonSystemMessages {
		...
		case "assistant":
			...
			for _, tc := range msg.ToolCalls {
				input := parseToolArgumentsToMap(tc.Function.Arguments)
				toolUses = append(toolUses, KiroToolUse{
					ToolUseID: tc.ID,
	...
	maxTokens := normalizeOpenAIMaxTokens(req.MaxTokens)
	if maxTokens > 0 || req.Temperature > 0 || req.TopP > 0 {
		payload.InferenceConfig = &InferenceConfig{
			MaxTokens:   maxTokens,
			Temperature: req.Temperature,
			TopP:        req.TopP,
...
func extractOpenAIResponseFormatHint(responseFormat map[string]interface{}) string {
	...
	case "json_schema":
		if schema, ok := responseFormat["json_schema"].(map[string]interface{}); ok {
			if raw, err := json.Marshal(schema["schema"]); err == nil {
...
func parseToolArgumentsToMap(toolArgs string) map[string]interface{} {
	...
	if err := json.Unmarshal([]byte(trimmed), &inputMap); err == nil && inputMap != nil {
		return inputMap
	}
	var raw interface{}
...
// buildFinalUserContent chooses the outgoing user message body. Orphan tool
// results (no matching last-assistant tool turn) must be narrated into text
// even when images are attached; otherwise image-only normalization would drop
// the tool output text.
func buildFinalUserContent(currentContent string, currentImages []KiroImage, currentToolResults []KiroToolResult, keepCurrentToolResults bool) string {
	if len(currentToolResults) > 0 && !keepCurrentToolResults {
		return buildToolResultsContinuation(currentToolResults)
	}
...
func convertOpenAITools(tools []OpenAITool) ([]KiroToolWrapper, map[string]string) {
	...
	for _, tool := range tools {
		...
		wrapper.ToolSpecification.Description = normalizeToolDesc(desc, sanitized)
