...
func isQuotaError(err error) bool {
	if kiroErr, ok := asKiroAPIError(err); ok {
		return kiroErr.StatusCode == 429
	}
	return isQuotaErrorMessage(err.Error())
}

func isAuthError(err error) bool {
	if kiroErr, ok := asKiroAPIError(err); ok {
		return kiroErr.StatusCode == 401 || kiroErr.StatusCode == 403
	}
	return isAuthErrorMessage(err.Error())
}

func isOverageError(err error) bool {
	if kiroErr, ok := asKiroAPIError(err); ok {
		return kiroErr.StatusCode == 402
	}
	return isOverageErrorMessage(err.Error())
}
