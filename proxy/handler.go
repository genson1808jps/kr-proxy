...
func (h *Handler) handleModels(w http.ResponseWriter, r *http.Request) {
	...
	models = appendKiroPrefixedModels(models)
...
// appendKiroPrefixedModels adds kr/<id> aliases for each listed model so clients
// can opt into the namespace without breaking bare id usage.
func appendKiroPrefixedModels(models []map[string]interface{}) []map[string]interface{} {
	if len(models) == 0 {
		return models
	}
	prefix := kiroModelPrefix
	prefixed := make([]map[string]interface{}, 0, len(models))
	for _, m := range models {
		id, _ := m["id"].(string)
		if id == "" || strings.HasPrefix(strings.ToLower(id), prefix) {
			continue
		}
...
func (h *Handler) handleClaudeStream(w http.ResponseWriter, payload *KiroPayload, model string, thinking bool, thinkingOpts claudeThinkingResponseOptions, estimatedInputTokens int, cacheProfile *promptCacheProfile, apiKeyID string) {
	...
	for attempt := 0; attempt < maxAccountRetryAttempts; attempt++ {
		...
		if err != nil {
			...
			_, errType, msg := upstreamClientError(err, "claude")
	...
	h.sendClaudeErrorFromUpstream(w, lastErr)
}
...
func (h *Handler) sendClaudeErrorFromUpstream(w http.ResponseWriter, err error) {
	status, errType, msg := upstreamClientError(err, "claude")
...
func (h *Handler) handleOpenAIStream(w http.ResponseWriter, payload *KiroPayload, model string, thinking bool, estimatedInputTokens int, apiKeyID string) {
	...
	for attempt := 0; attempt < maxAccountRetryAttempts; attempt++ {
		...
		if err != nil {
			...
			h.sendOpenAIStreamErrorFromUpstream(w, flusher, err)
			return
		}
...
func (h *Handler) sendOpenAIErrorFromUpstream(w http.ResponseWriter, err error) {
	status, errType, msg := upstreamClientError(err, "openai")
	h.sendOpenAIError(w, status, errType, msg)
}

func (h *Handler) sendOpenAIStreamErrorFromUpstream(w http.ResponseWriter, flusher http.Flusher, err error) {
	status, errType, msg := upstreamClientError(err, "openai")
	chunk := map[string]interface{}{
		"error": map[string]interface{}{
			"type":    errType,
