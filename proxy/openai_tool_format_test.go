...
func TestConvertOpenAIToolsEmitsNonEmptyNames(t *testing.T) {
	...
	if wrappers[0].ToolSpecification.Name != "execCommand" {
		t.Fatalf("expected execCommand preserved, got %q", wrappers[0].ToolSpecification.Name)
	}
	if wrappers[1].ToolSpecification.Name != "updatePlan" {
		t.Fatalf("expected updatePlan preserved, got %q", wrappers[1].ToolSpecification.Name)
	}
	if nameMap["execCommand"] != "exec_command" {
		t.Fatalf("expected exec_command in name map, got %#v", nameMap)
	}
	if nameMap["updatePlan"] != "update_plan" {
		t.Fatalf("expected update_plan in name map, got %#v", nameMap)
	}
}
