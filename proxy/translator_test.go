...
func TestConvertOpenAIToolsSanitizesSchemaAndDescription(t *testing.T) {
	...
	if tools[0].ToolSpecification.Name != "readFile" {
		t.Fatalf("expected readFile after sanitize, got %q", tools[0].ToolSpecification.Name)
	}
	if nameMap["readFile"] != "read_file" {
		t.Fatalf("expected name map readFile->read_file, got %#v", nameMap)
	}
...
func TestParseModelAndThinking(t *testing.T) {
	tests := []struct {
		...
		// Optional kr/ namespace prefix (case-insensitive).
		{"kr prefix dash form", "kr/claude-opus-4-8", "claude-opus-4.8", false},
		{"kr prefix dot form", "kr/claude-opus-4.8", "claude-opus-4.8", false},
		{"kr prefix uppercase", "KR/claude-sonnet-4.6", "claude-sonnet-4.6", false},
		{"kr prefix with thinking suffix", "kr/claude-opus-4.8-thinking", "claude-opus-4.8", true},
